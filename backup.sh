...
ssh -i ~/.ec2/ridemissionkey.pem ec2-user@50.19.106.243 ./backup.sh
...
mongo localhost:27017/service-development --quiet --eval "var dropped = db.dropDatabase()" >> backups.log 2>&1

# restore the local database
echo "restoring local database from remote backup..."
mongorestore -d service-development -h localhost:27017 service-db >> backups.log 2>&1
