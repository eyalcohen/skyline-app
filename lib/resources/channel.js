...
exports.routes = function (app) {
  ...
  app.put('/api/channels/:id', function (req, res) {
    ...
    Step (
      ...
      function (err, doc) {
        if (com.error(err, req, res, doc, 'channel')) return;
        ...
        db.Datasets.update({_id: doc.parent_id}, {$set: {updated: new Date}}, this.parallel());
      },
      ...
      function (err, stat) {
        if (com.error(err, req, res, stat, 'channel')) return;
        db.Channels.read({_id: db.oid(req.params.id)}, this);
      },
      function (err, doc) {
        if (com.error(err, req, res, doc, 'channel')) return;
        com.index(app.get('redis'), 'channels', doc, ['humanName'], this);
      },
      function (err) {
  ...
  app.post('/api/channels/search/:s', function (req, res) {
    ...
    Step(
      ...
      // Perform the search.
      function () {
        com.search(app.get('redis'), 'channels', req.params.s, 250, this);
      },
