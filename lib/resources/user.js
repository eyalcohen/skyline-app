...
function indexUser(user) {
  if (user.displayName && user.displayName !== ''
      && user.displayName.match(/\w+/g))
    search.index(user.displayName, user._id.toString());
  if (user.username.match(/\w+/g))
    search.index(user.username, user._id.toString());
}
        ...
        Step(
          ...
          function (err) {
            ...
            _.each(views, function (v) {
              ...
              // Publish removed status.
              pubsub.publish('views', 'views.removed', {id: v._id.toString()});

              db.Comments.remove({parent_id: v._id}, parallel());
  ...
  app.post('/api/users/:un/follow', function (req, res) {
    ...
    db.Users.read({username: req.params.un}, function (err, user) {
      ...
      db.Subscriptions.create({
        ...
        subscribee_id: user._id,
        meta: {style: 'follow', type: 'user'},
        mute: false
      }, function (err, sub) {
