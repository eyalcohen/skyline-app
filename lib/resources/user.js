...
function indexUser(user) {
  com.index(app.get('redis'), 'users', user, ['username', 'displayName'], function() {});
}

/*
 * Subscribe user to library.
 */
function subscribeToLibrary(user, cb) {
  db.Users.read({username: 'library'}, function (err, library) {
    if (err) return cb(err);
    if (!library) return cb('Could not find library');

    // Create subscription.
    app.get('pubsub').subscribe(user, library, {style: 'follow', type: 'user'},
        function (err, sub) {
      if (err) return cb(err);
...
var createUser = exports.createUser = function(userProps, cb) {
  if (!userProps || !userProps.newusername || !userProps.newemail
      || !userProps.newpassword) {
    return cb({message: 'User invalid'});
  }

  // Check details.
  userProps.newusername = _.slugify(userProps.newusername).substr(0, 30);
  if (userProps.newusername.length < 4) {
    return cb({message: 'Username too short'});
  }
  if (userProps.newpassword.length < 7) {
    return cb({message: 'Password too short'});
  }

  // Check blacklist.
  if (_.contains(BLACKLIST, userProps.newusername)) {
  ...
  var props = {
    provider: 'local',
    username: userProps.newusername,
    displayName: userProps.newusername,
    emails: [{value: userProps.newemail}],
    primaryEmail: userProps.newemail,
    salt: com.salt(),
  ...
  db.Users.create(props, function (err, user) {
    ...
    // Sub user to library.
    subscribeToLibrary(user, function (err) {
      cb(err, user);
    });
  });
};
...
exports.routes = function (app) {
  ...
  app.post('/api/users/search/:s', function (req, res) {
    ...
    Step(
      ...
      // Perform the search.
      function () {
        com.search(app.get('redis'), 'users', req.params.s, 20, this);
      },
