    ...
    Step (
      function () {
        switch (e.action_type) {
          ...
          case 'dataset':
            var _after = _.bind(function (err, access) {
              if (!access) {
                e._reject = true;
                return this();
              }
            ...
            com.hasAccess(requestor, e.action, _after)
            break;
          ...
          case 'view':
            Step(
              function () {
                ...
                // Check access.
                var allow = true;
                var _cb = _.after(_.size(e.action.datasets), _.bind(function (err) {
                  this(err, allow);
                }, this));
                _.each(e.action.datasets, function (meta, did) {
