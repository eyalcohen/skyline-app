    ...
    Step(
      ...
      function (err, datasetDoc, userDoc) {
        ...
        if (datasetDoc && !userDoc) {
          ...
          if (!userDoc) {
            db.Users.create({username: userId});
          }
          var props = {
            _id: datasetId,
            public: 'false',
      ...
      function (err) {
        if (err) {
          console.log('Error while processing PUT /samples request: ' +
              (err.stack || err));
