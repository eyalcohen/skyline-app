...
exports.routes = function (app) {
  ...
  app.put('/api/datasets/:id', function (req, res) {
    ...
    Step (
      function() {
        db.Datasets.read({_id: Number(req.params.id)}, this);
      },
      function (err, doc) {
        if (com.error(err, req, res, doc, 'dataset')) return;
        ...
        db.Datasets.update({_id: Number(req.params.id)}, {$set: props}, this);
      },
      ...
      function (err, doc) {
        ...
        com.index(app.get('redis'), 'datasets', doc,
          ['title', 'sources', 'tags'], this.parallel());
        com.index(app.get('redis'), 'datasets', doc,
          ['title'], {strategy: 'noTokens'}, this.parallel());
      },
      function (err) {
  ...
  // app.put('/api/datasetss', function (req, res) {

  //   var params = {
  //     where: 'uploads-skyline',
  //     key: '@' + 'userId' + '-' + 'datasetId' + '-' + (new Date().valueOf()) + '.json',
  //     data: req.rawBody,
  ...
  app.put('/api/datasets', function (req, res) {
    ...
    Step(
      ...
      function getDataset(err) {
        ...
        var params = {
          where: 'uploads-skyline',
          key:  '@' + userId + '-' + datasetId + '-' + (new Date().valueOf()),
          data:  req.rawBody,
