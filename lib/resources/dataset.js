...
exports.routes = function (app) {
  ...
  var datasetSearch = app.get('reds').createSearch('datasets');
  var userSearch = app.get('reds').createSearch('users');
  var channelSearch = app.get('reds').createSearch('channels');
  ...
  app.post('/api/datasets/search/:s', function (req, res) {
    ...
    // Perform the search.
    //datasetSearch.query(req.params.s).end(function (err, ids) {
    console.log(req.params.s);
    app.get('redis').zrangebylex(
      'datasets',
      '[' + req.params.s,
      // Don't know why the 0xFF works... saw in a gist but can't find doc
      '[' + req.params.s + String.fromCharCode(0xFF),
      function (err, ids) {
...
*/
      ...
      function (err, doc) {
        ...
        com.index(userSearch, userDoc, ['userName', 'displayName']);
        com.index(datasetSearch, datasetDoc, ['title', 'sources', 'tags']);
        if (newDataset) {
        ...
        _.each(schema, function (s) {
          ...
          db.Channels.create(s, function (err, channel) {
            if (err && err.code === 11000) {
              ...
              // Index for search.
              com.index(channelSearch, channel, ['humanName']);
              _this();
            }
          });
        });
