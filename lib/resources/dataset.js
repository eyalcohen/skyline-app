...
exports.routes = function (app) {
    ...
    db.Datasets.read({_id: Number(req.params.id)},
      ...
      Step(
        function () {
          ...
          // Check access.
          com.hasAccess(req.user, doc, this);
        },
        function (err, allow) {
          if (com.error(err, req, res)) return;
          if (!allow) {
  ...
  app.put('/api/datasets/:id', function (req, res) {
    ...
    Step (
      ...
      function (err, stat) {
        if (com.error(err, req, res, stat, 'dataset')) return;
      ...
      function (err, doc) {
        if (com.error(err, req, res, doc, 'dataset')) return;
        com.index(app.get('redis'), 'datasets', doc,
  ...
  app.put('/api/datasets', function (req, res) {
    ...
    Step(
      ...
      function (err, doc) {
        ...
        com.index(app.get('redis'), 'users', userDoc,
          ['username', 'displayName'], this.parallel());
        com.index(app.get('redis'), 'datasets', datasetDoc,
          ['title', 'source', 'tags'], this.parallel());
        com.index(app.get('redis'), 'datasets', datasetDoc,
          ['title', 'source'], {strategy: 'noTokens'}, this.parallel());
...
*/
  ...
  app.post('/api/datasets/:id/watch', function (req, res) {
    ...
    db.Datasets.read({_id: Number(req.params.id)}, function (err, doc) {
      ...
      // Check access.
      com.hasAccess(req.user, doc, function (err, allow) {
        if (com.error(err, req, res)) return;
        if (!allow) {
          return com.error(null, req, res, undefined, 'dataset');
        }
  ...
  app.post('/api/datasets/:id/unwatch', function (req, res) {
    ...
    db.Datasets.read({_id: Number(req.params.id)}, function (err, doc) {
      ...
      com.hasAccess(req.user, doc, function (err, allow) {
        ...
        // Remove subscription.
        pubsub.unsubscribe(req.user, doc, function (err, sub) {
          if (com.error(err, req, res, sub, 'subscription')) return;
