...
exports.routes = function (app) {
  ...
  // Create
  // Note: New datasets handled by lib/client.js,
  // forks handled here.
  app.post('/api/datasets', function (req, res) {
    if (!req.user || !req.body || !req.body.title || !req.body.parent_id)
      return res.send(403, {error: 'Dataset invalid'});
    ...
    Step(
      function () {
        ...
        // Get fork parent.
        db.Datasets.read({_id: props.parent_id},
            {inflate: {author: profiles.user}}, this);
      },
      function (err, parent) {
        if (com.error(err, req, res, parent, 'dataset')) return;

        // Copy parent data.
        props.meta = parent.meta;
        props.client_id = parent.client_id;

        // Handle public.
        props.public = props.public === 'true' || props.public;

        // Create the dataset.
        db.Datasets.create(props, {force: {_id: 1},
            inflate: {author: profiles.user}}, function (err, doc) {
          if (com.error(err, req, res)) return;

          // Index for search.
          // if (doc.title !== '' && doc.title.match(/\w+/g))
          //   self.search.index(doc.title, doc._id);
          // if (doc.source !== '' && doc.source.match(/\w+/g))
          //   self.search.index(doc.source, doc._id);
          // _.each(doc.tags, function (t) {
          //   if (t.match(/\w+/g)) self.search.index(t, doc._id);
          // });
          // var fileName = _.strLeft(doc.file.name, '.');
          // if (fileName !== '' && fileName.match(/\w+/g))
          //   self.search.index(fileName, doc._id);

          // Inc author dataset count.
          db.Users._update({_id: doc.author._id}, {$inc: {dcnt: 1}},
          ...
          Step(
            function () {
              ...
              // Copy samples.
              var next = _.after(Samples.bucketThresholds.length
                  + Samples.syntheticDurations.length, this);
              _.each(Samples.bucketThresholds, function (level) {
                db['Real_' + level + 's'].list({did: parent._id},
                    function (err, samples) {
                  if (com.error(err, req, res)) return;
                  if (samples.length === 0) return next();
                  var _next = _.after(samples.length, next);
                  _.each(samples, function (s) {
                    delete s._id;
                    s.did = doc._id;
                    if (s.chn.indexOf('__' + parent._id) !== -1)
                      s.chn = _.strLeft(s.chn, '__') + '__' + s.did;
                    if (s.val.channelName)
                      s.val.channelName =
                          _.strLeft(s.val.channelName, '__') + '__' + s.did;
                    db['Real_' + level + 's'].insert(s, _next);
                  });
                });
              });
              _.each(Samples.syntheticDurations, function (duration) {
                db['Syn_' + duration + 's'].list({did: parent._id},
                    function (err, samples) {
                  if (com.error(err, req, res)) return;
                  if (samples.length === 0) return next();
                  var _next = _.after(samples.length, next);
                  _.each(samples, function (s) {
                    ...
                    if (s.chn.indexOf('__' + parent._id) !== -1)
                      s.chn = _.strLeft(s.chn, '__') + '__' + s.did;
                    if (s.val && s.val.channelName)
                      s.val.channelName =
                          _.strLeft(s.val.channelName, '__') + '__' + s.did;
                    db['Syn_' + duration + 's'].insert(s, _next);
                  });
                });
              });
            },
            function (err) {
              ...
              var target = {
                ...
                i: doc._id.toString(),
                n: doc.title,
                s: [req.user.username, doc._id].join('/'),
                l: doc.public === false,
                p: {
                  ...
                  n: parent.title,
                  s: [parent.author.username, parent._id].join('/'),
              ...
              pubsub.subscribe(req.user, parent, {style: 'watch', type: 'dataset'});
              pubsub.subscribe(req.user, doc, {style: 'watch', type: 'dataset'});

              // Publish new dataset.
              pubsub.publish('datasets', 'dataset.new', doc);
