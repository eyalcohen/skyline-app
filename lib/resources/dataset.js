...
exports.routes = function () {
  ...
  // Example: curl 'http://localhost:8080/datasets/123456?beg=1309914019674000&end=1309916383000000&chan=$beginDate&chan=$beginTime&chan=$beginAbsTime&chan=$duration&chan=$beginRelTime&chan=$endRelTime&chan=gps.speed_m_s&chan=gps.latitude_deg&chan=gps.longitude_deg&chan=gps.altitude_m&chan=accel.x_m_s2&minmax'
  app.get('/api/datasets', function (req, res, next) {
    ...
    Step(
      ...
      function fetchData() {
        // Server URI
        var uri = 'http://'
            + app.get('SAMPLES_HOST')
            + ':' + app.get('SAMPLES_PORT');
        ...
        channels.forEach(function (channelName) {
          var reqParams = {
            ...
            qs: {
              ...
              downsample: resample ? resample : null
            },
            method: 'get'
          };
          request(reqParams, group());
        });
      },

      // every response has a set of samples arrange as
      // {time, avg, min, max}
      function organize(err, responses) {
        if (err) return this(err);

        var rows = {};

        var sampleSet = _.map(responses, function(response) {
          return JSON.parse(response.body).samples;
        });

        //O(n*m)
        _.each(sampleSet, function(samples, idx) {
          _.each(samples, function(sample) {
            if (!rows[sample.time]) rows[sample.time] = {};
            rows[sample.time][channels[idx]] = sample;
          });
        });

        return this(null, rows);
      },

      function writeData(err, rows) {
        if (err) return this(err);

        var timestamps = _.keys(rows).sort(function (a, b) { return a - b; });

        var header = [];
        header.push('Date');
        channels.forEach(function (channelName) {
          header.push(channelName.split('__')[0]);
          if (getMinMax) {
            header.push('min');
            header.push('max');
          }
        ...
        _.each(timestamps, function(t) {
          var line = [];
          // print an ISO string but remove the special characters
          line.push(new Date(Number(t)).toISOString().replace(/T/, ' ').replace(/\..+/, ''));
          _.each(channels, function(col) {
            var sample = rows[t][col];
            line.push(sample ? sample.avg : '');
            if (getMinMax) {
              line.push(sample ? sample.min : '');
              line.push(sample ? sample.max : '');
            }
          });
          csv.write(line);
        });
  ...
  app.get('/api/datasets/:id', function (req, res) {
    ...
    db.Datasets.read({_id: Number(req.params.id)},
      ...
      if (errorHandler(err, req, res, doc, 'dataset')) return;

      Step(
        function () {

          // Check access.
          hasAccess(db, req.user, doc, this);
        },
        function (err, allow) {
          if (errorHandler(err, req, res)) return;
          if (!allow) {
            return errorHandler(null, req, res, undefined, 'dataset');
          }
  ...
  app.put('/api/datasets/:id', function (req, res) {
    ...
    if (props.schedule) {
      props.schedule = Number(props.schedule);
    }
    ...
    Step (
      ...
      function (err, doc) {
        ...
        db.Datasets.update({_id: did}, {$set: props}, this);
      },
      function (err, stat) {
        if (errorHandler(err, req, res, stat, 'dataset')) return;

        // If needed, update event privacy.
        if (props.public !== undefined) {
          db.Events.update({action_id: did}, {$set: {public: props.public}},
      ...
      function (err, doc) {
        if (errorHandler(err, req, res, doc, 'dataset')) return;
        cache.index('datasets', doc, ['title', 'source', 'tags'],
            this.parallel());
        cache.index('datasets', doc, ['title', 'source'],
        ...
        Step(
          ...
          function (err, notes, channels) {
            ...
            _.each(channels, function (c) {
              events.publish('channel', 'channel.removed', {data: {id: c._id.toString()}});
            });
          ...
          function (err) {
            ...
            events.publish('dataset', 'dataset.removed', {data: {id: doc._id.toString()}});
  ...
  app.post('/api/datasets/search/:s', function (req, res) {
    ...
    Step(
      ...
      // Perform the search.
      function () {
        cache.search('datasets', req.params.s, 20, this);
      },
      ...
      function (err, ids) {
        ...
        db.Datasets.list(query, {sort: {vcnt: 1, title: -1},
    ...
    Step(
      ...
      function getDataset(err) {
        ...
        var params = {
          ...
          key:  '@' + userId + '-' + datasetId + '-' + now + fileExt,
          data:  req.rawBody,
      ...
      function (err, doc) {
        ...
        cache.index('users', userDoc, ['username', 'displayName'],
            this.parallel());
        cache.index('datasets', datasetDoc, ['title', 'source', 'tags'],
            this.parallel());
        cache.index('datasets', datasetDoc, ['title', 'source'],
            {strategy: 'noTokens'}, this.parallel());

        if (newDataset) {

          // Publish dataset.
          events.publish('dataset', 'dataset.new', {
            data: datasetDoc,
            event: {
          ...
          events.subscribe(userDoc, datasetDoc,
...
*/
        ...
        _.each(schema, function (s) {
          ...
          db.Channels.create(s, function (err, channel) {
            if (err && err.code === 11000) {
              ...
              // Index for search.
              cache.index('channels', channel, ['humanName'], _this);
            }
          });
        });
      },
      function (err) {
        ...
        // Update dataset updated time.
        db.Datasets.update({_id: datasetDoc._id}, {}, this);
      },
  ...
  // Watch
  app.post('/api/datasets/:id/watch', function (req, res) {
    if (!req.user) {
      return res.send(403, {error: {message: 'User invalid'}});
    }
    ...
    db.Datasets.read({_id: Number(req.params.id)}, function (err, doc) {
      ...
      hasAccess(db, req.user, doc, function (err, allow) {
        ...
        // Create subscription.
        events.subscribe(req.user, doc, {style: 'watch', type: 'dataset'},
            function (err, sub) {
  ...
  // Unwatch
  app.post('/api/datasets/:id/unwatch', function (req, res) {
    if (!req.user) {
      return res.send(403, {error: {message: 'User invalid'}});
    }
