...
exports.routes = function (app) {
  ...
  // List
  app.post('/api/notes/list', function (req, res) {
    ...
    db.Notes.list(query, {sort: {created: 1}, limit: limit,
        ...
        function (err, pparent) {
          ...
          db.Notes.create(props, {inflate: {author: profiles.user}},
            ...
            pubsub.notify({
              ...
              public: type !== 'note' ?
                  parent.public !== false: pparent.public !== false
            }, '"' + doc.body + '"');
            ...
            // Publish note.
            pubsub.publish('notes', 'note.new', doc);
  ...
  app.get('/api/notes/:id', function (req, res) {
    ...
    db.Notes.read({_id: db.oid(req.params.id)}, function (err, doc) {
      ...
      // Fill note replies.
      db.fill(doc, 'Notes', 'parent_id', {sort: {created: -1}, limit: 5,
  ...
  app.delete('/api/notes/:id', function (req, res) {
    ...
    function _delete(doc, cb) {
      Step(
        function () {
          ...
          // Remove events where note is action.
          db.Events.remove({action_id: doc._id}, this.parallel());

          // Finally, remove the note.
          db.Notes.remove({_id: doc._id}, this.parallel());
        },
        function (err) {
          cb(err);
          ...
          // Publish removed status.
          if (!err)
            pubsub.publish('notes', 'note.removed', {id: doc._id});
        }
      );
    }
    ...
    Step(
      function () {
        ...
        db.Notes.list({parent_id: id}, this.parallel());
      },
      function (err, note, notes) {
        if (com.error(err, req, res, note, 'note')) return;
        notes.push(note);
        var _this = _.after(notes.length, this);
        _.each(notes, function (com) {
          _delete(com, _this);
        });
      },
      function (err) {
