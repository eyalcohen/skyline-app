  ...
  app.get('/service/home.profile', function (req, res) {
    ...
    function getEvents(cb) {
      ...
      db.Subscriptions.list({subscriber_id: req.user._id, mute: false,
          'meta.style': 'follow'}, function (err, subs) {
        if (err) return cb(err);

        if (subs.length === 0) return cb(null, []);
        Step(
          function () {
            var events = [];
            ...
            _.each(subs, function (s) {
              ...
              db.Events.list({actor_id: s.subscribee_id},
                  {sort: {created: -1}, limit: 5}, _.bind(function (err, docs) {
                if (err) return this(err);

                // Gather events.
                _this(null, events.push.apply(events, docs));
              }, this));
            });
          },
          function (err, events) {
