...
exports.routes = function (app) {
   ...
   */
  function getSidebarLists(user, cb) {
    if (!user) {
      return cb(null, {datasets: [], views: []});
    }
    Step(
      function () {
  ...
  app.get('/service/dashboard.profile', function (req, res) {
    ...
    Step(
      function () {
        ...
        // Get sidebar.
        getSidebarLists(req.user, this.parallel());

        // Count unread notifications.
        db.Notifications.count({subscriber_id: req.user._id, read: false},
            this.parallel());
      },
      function (err, events, lists, unread) {
  ...
  app.get('/service/user.profile/:un', function (req, res) {
    ...
    db.Users.read({username: req.params.un.toLowerCase()},
      ...
      Step(
        ...
        function (err, sub) {
          if (com.error(err, req, res)) return;
          var allow = user.config.privacy.mode.toString() === '0'
          ...
          Step(
            function () {
              ...
              // Get sidebar.
              getSidebarLists(user, this.parallel());
            },
            function (err, events, lists) {
              if (com.error(err, req, res)) return;
              if (!events) {
                events = {items: []};
              }
              if (!lists) {
                lists = {datasets: [], views: []};
              }
