...
exports.routes = function () {
  ...
  function getSidebarLists(user, requestor, cb) {
    ...
    function hasViewAccess(view, list, done) {
      hasAccess(db, requestor, view, function (err, allow) {
        ...
        if (allow) {
          ...
          _.each(view.datasets, function (meta, did) {
            db.Datasets.read({_id: Number(did)}, function (err, d) {
              hasAccess(db, requestor, d, function (err, _allow) {
      ...
      function (err, datasets, views, followers, followees) {
        ...
        Step(
          ...
          function (err, _views) {
            ...
            _.each(datasets, function (d) {
              hasAccess(db, requestor, d, function (err, allow) {
  ...
  function getPublicSidebarLists(cb) {
    ...
    Step(
      ...
      function (err, datasets, count) {
        ...
        lists.library.items = datasets;
        lists.library.count = count;

        // Get the 'end' time of all channels in streaming Datasets
        // to get a 'last seen'
        var streaming = _.compact(_.reject(lists.library.items, function (d) {
          return d.file;
        }));
        Step(
          function () {
  ...
  app.get('/service/static', function (req, res) {
    res.send(sutil.client({user: req.user, content: {page: null}}));
  });
  ...
  app.get('/service/dashboard', function (req, res) {
    ...
    var actions = parseEventActions(req, req.user ?
        ['dataset', 'view', 'note', 'comment']: ['view']);
    ...
    Step(
      ...
      function (err, feed, publicLists, lists, unread) {
  ...
  app.get('/service/notifications', function (req, res) {
    if (!req.user) {
      return res.send(sutil.client({user: null, content: {page: null}}));
    }
    ...
    Step(
      ...
      function (err, notifications, publicLists, lists, unread) {
        if (errorHandler(err, req, res)) return;
        lists = _.extend(lists, publicLists);
        ...
        if (state.key) {
          ...
          db.Users.read({username: state.key.un}, _.bind(function (err, user) {
            ...
            db.Views.read({slug: state.key.slug, author_id: user._id},
              ...
              if (errorHandler(err, req, res, view, 'view')) return;

              // Check access.
              hasAccess(db, req.user, view, _.bind(function (err, allow) {
  ...
  app.get('/service/dataset/:id', function (req, res) {
    ...
    db.Datasets.read({_id: Number(req.params.id)},
      ...
      if (errorHandler(err, req, res, dataset, 'dataset')) return;          

      Step(
        function () {

          // Check access.
          hasAccess(db, req.user, dataset, this);
        },
        function (err, allow) {
          if (errorHandler(err, req, res)) return;
          if (!allow) {
      ...
      db.Views.read({slug: req.params.slug, author_id: user._id},
        ...
        Step(
          function () {
            ...
            // Check access.
            hasAccess(db, req.user, view, this);
          },
          function (err, allow) {
            if (errorHandler(err, req, res)) return;
            if (!allow) {
  ...
  app.get('/service/library', function (req, res) {
    ...
    db.Users.read({username: 'library'}, function (err, library) {
      if (errorHandler(err, req, res, library, 'library')) return;
  ...
  app.get('/service/settings', function (req, res) {
    ...
    res.send(sutil.client({user: req.user, content: {page: req.user}}));
  });
        ...
        Step(
          ...
          function (err) {
            ...
            res.render('static', {
              key: 'library',
              title: 'Library',
              body: _.template(library_static, {datasets: datasets, static: true}),
              root: app.get('ROOT_URI')
            });
          }
        );
  ...
  app.get('/trending', _.bind(handler, undefined, function (req, res) {
    ...
    Step(
      ...
      function (err, feed, publicLists) {
        ...
        var html = '<h2>Public content now trending on skyline:</h2>';
        ...
        html += '<h2>Recently added public datasets:</h2>';
        _.each(publicLists.library.items, function (d) {
