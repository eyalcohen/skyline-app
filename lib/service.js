...
exports.routes = function (app) {
  ...
  function getSidebarLists(user, requestor, cb) {
    var lists = {datasets: {items: []}, views: {items: []},
      followers: {items: []}, followees: {items: []}};
    if (!user) {
    ...
    Step(
      ...
      function (err, _datasets, _views, followers, followees) {
      ...
      function (err, docs) {
        ...
        _.each(views, function (v) {
          com.hasAccess(requestor, v, function (err, allow) {
            if (err) return _this(err);
            if (allow) {
              var __this = _.after(_.size(v.datasets), function (err) {
                if (err) return _this(err);
                if (allow) {
                  lists.views.items.push(v);
                }
                _this();
              });
              _.each(v.datasets, function (meta, did) {
      ...
      function (err) {
        ...
        lists.views.items.sort(function (a, b) {
          return b.created - a.created;
        });
