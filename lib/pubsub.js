    ...
    function () {
      ...
      if (props.target_id)
        db.Subscriptions.list({
          ...
          'meta.style': 'watch', mute: false
        }, {inflate: {subscriber: _.extend(_.clone(profiles.user),
            {primaryEmail: 1, config: 1})}}, this.parallel());
    },
    function (err, event, subs) {
      if (err) return cb ? cb(err): util.error(err);
      ...
      _.each(subs, function (sub) {
        ...
        db.Notifications.create(props, function (err, note) {
          ...
          note.event = event;
          self.publish('usr-' + sub.subscriber._id.toString(),
