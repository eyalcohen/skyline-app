...
Client.prototype.insertSamples = function (obj, cb) {
  ...
  Step (
    ...
    function indexDataset(err, doc) {
      ...
      // FIXME
      return this;
      self.cache.index(self.redis, 'datasets', doc,
    ...
    function indexChannels (err, channels) {
      ...
      _.each(channels, function (c) {
        self.cache.index(self.redis, 'channels', c, ['humanName'], null);
        self.cache.index(self.redis, 'channels', c, ['humanName'], {strategy: 'noTokens'});
      });
...
// As a first pass, this function just fetches the samples and counts
// them. If this becomes too slow, roll a custom function
...
Client.prototype.exportCalculations = function (did, channel, beg, end, cb) {
  ...
  request(reqParams, function(err, resp) {
    if (!err && resp && resp.body)
      try {
        var body = JSON.parse(resp.body);
        if (body.samples) {
          cb(null, body.samples.length);
          return;
        }
      } catch (e) {}
