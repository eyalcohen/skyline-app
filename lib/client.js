...
// Constructor
var Client = exports.Client = function (socket, opts) {
  this.socket = socket;
  ...
  this.socket.on('rpc', _.bind(function () {
    ...
    var user = this.socket.handshake.user ? this.socket.handshake.user.username : 'anon';
...
Client.prototype.sendPartialFile = function (data, cb) {
  ...
  if (chunk[id].currentSize === chunk[id].expectedSize) {
    ...
    var params = {
      ...
      key:  '@' + (self.socket.handshake.user.username || '') + '-' + chunk[id].file.name,
...
Client.prototype.insertSamples = function (obj, cb) {
  ...
  Step (
    ...
    function createSampleSet(err, rows) {
      if (err) return this(err);
      var dateColumn = chunk[id].dateColumn;
      _.each(rows, _.bind(function (row, index) {
        var time = new Date(row[dateColumn]).valueOf();
        firstBeg = Math.min(firstBeg, time);
        lastEnd = Math.max(lastEnd, time);
        if (time) {
          _.each(row, function (value, key) {
            if (key !== dateColumn // && payload.channels[key].enabled
              ...
              if (!isNaN(value)) {
                ++num;
                sampleSet[key].samples.push({time: time, value: value});
              }
            }
          });
        }
    ...
    function createSampleArray(err) {
      if (err) return this(err);
      ...
      _.each(columns, function (channelName) {
        var set = sampleSet[channelName];
        if (!set || !set.samples || set.samples.length === 0) return;
        ...
        var v = {
          beg: _.first(set.samples).time,
          end: _.last(set.samples).time,
          channelName: cn,
          humanName: humanName,
          type: 'float',
          merge: true,
    ...
    function writeDatasetDb(err, user) {
      if (err) return this(err);
    ...
    function indexDataset(err, doc) {
        ...
        // Index for search.
        com.index(self.redis, 'datasets', doc,
          ['title', 'source', 'tags'], this.parallel());
        com.index(self.redis, 'datasets', doc,
          ['title', 'source'], {strategy: 'noTokens'}, this.parallel());
    },

    function insertSamples(err) {
      var uri = 'http://'
        + self.app.get('SAMPLES_HOST')
        + ':' + self.app.get('SAMPLES_PORT');
      _.each(sampleSet, function(val, key) {
        sampleSet[key].dataset = dataset._id;
      });
      request( {
        uri: uri,
        method: 'post',
    ...
    function insertChannels(err, resp, body) {
      if (err || resp.statusCode !== 200) return this(err);
      ...
      _.each(schema, function (s) {
        ...
        db.Channels.create(s, group());
      });
    },
    function indexChannels (err, channels) {
      if (err) return this(err);
      if (dataset.channels.length < 5 || dataset.author.role === 2) {
        dataset.channels.concat(channels);
      }

      // Index for search.  Don't care if this fails, so no callback.
      _.each(channels, function (c) {
        com.index(self.redis, 'channels', c, ['humanName'], null);
        com.index(self.redis, 'channels', c, ['humanName'], {strategy: 'noTokens'});
      });
    ...
    function (err) {
      ...
      if (err) {
        // FIXME: Need to remove all samples, datasets, channels and indexes
