...
Client.prototype.previewFileInsertion = function (obj, cb) {
  ...
  Step(
    ...
    function (err, rows, nn) {
      ...
      if (obj.skipHeaderRows >= nn) {
        delete obj.skipHeaderRows;
      }

      if (!obj.dateColumn || !obj.dateFormat) {
        timecolGuess = parseTimecol(rows[obj.skipHeaderRows || 0]);
      ...
      var parsedDateColumn = _.each(rows, function(r, idx) {
        var m;
        if (obj.dateFormat)
      ...
      var ret = {
        fileId: obj.fileId,
        header: rows[obj.skipHeaderRows || 0],
