...
var SampleMath = require('./sample_math').SampleMath;
...
// Constructor
var Client = exports.Client = function (socket, pubsub, samples, reds, redisclient) {
  this.socket = socket;
  this.pubsub = pubsub;
...
/*
  options {
    operation: {
      type: <String>,     eg simpleMovingAverage
      metric: <Number>,  eg 10 for 10 moving average samples
    },
    duration: <Number>,  ie the resample rate of the samples
    beg: <Number>,
    end: <Number>
  }
*/
Client.prototype.getMath = function (did, channelName, options, cb) {
  var key = [
    channelName,
    options.duration,
    options.operation.type,
    options.operation.metric
  ].join(':');

  var self = this;
  var duration = options.duration;
  var operation = options.operation;
  var beg = options.beg, end = options.end;
  ...
  Step(
    // 1.  Check keystore for channelName/duration/operation
    function(err) {
      // TODO: Pipeline
      self.redisclient.lrange(key, 0, -1, this)
    },
    // 2.. Check reply, if we have one return early
    function(err, reply) {
      if (err) return this(err);
      if (reply.length > 0) {
        console.log('Redis cache fetch time', Date.now()-now);
        cb(err, reply);
      } else {
        console.log('fetching data from db');
        self.fetchSamples(did, channelName, {}, this);
      }
    },
    function(err, data) { 
      if (err) return this(err);
      var sampleSet = data.samples;
      results = new SampleMath(sampleSet, self.samples, options);

      var multi = self.redisclient.multi();
      _.each(results.vals, function (r) {
        multi.rpush(key, r);
      });
    ...
    function(err) {
      console.log(err);
      console.log('Db fetch time', Date.now()-now);
      cb(err, results);
    }
  );
};
