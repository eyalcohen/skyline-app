  ...
  Step (
    ...
    function (err) {
      ...
      Step(
        ...
        // Step2.  Add to the channels collection
        function (err, doc) {
          if (err) return next(err);
          user = doc;
          _.each(schema, _.bind(function(s) {
            //if (!this.channelName) ;
            _.extend(s, { parent_id: did, _id: com.createId_32() });
            db.Channels.create(s, {force: {_id: 1, parent_id: 1}}, this.parallel());
          }, this));
        },
        // Step3.  Add to the dataset collection
        function (err) {
          if (err) return next(err);
