...
Client.prototype.getMath = function (did, channelName, options, cb) {
  console.log('here');
  var key = [channelName, options.duration, options.operation].join(':');
  var self = this;
  ...
  Step(
    ...
    // Check replay, if we have one return 
    function(err, reply) {
    ...
    function(err, fetchedSamples) { 
      console.log(err, fetchedSamples);
      samples = fetchedSamples;
      if (err) return this(err);
      _.each(samples, function (s) {
        console.log(s);
        self.redisclient.lpush(key, s.val, this.parallel());
      }, this);
    },
    function(err) {
      cb(err, samples);
