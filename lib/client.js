    ...
    function (err, rows, nn) {
      ...
      _.each(rows, _.bind(function (row, index) {
        ...
        function getDate(r) {
          ...
          if (r[timecol] === '') return;
          if (dateHint === 'year')
            d = d.setUTCFullYear(r[timecol]).valueOf();
          else if (dateHint === 'month')
            d = d.setUTCMonth(r[timecol]).valueOf();
          else if (dateHint === 'day')
            d = d.setUTCDate(r[timecol]).valueOf();
          else if (dateHint === 'hour')
            d = d.setUTCHours(r[timecol]).valueOf();
          else if (dateHint === 'minute')
            d = d.setUTCMinutes(r[timecol]).valueOf();
          else if (dateHint === 'second')
            d = d.setUTCSeconds(r[timecol]).valueOf();
          else if (dateHint === 'millisecond')
            d = d.setUTCMilliseconds(r[timecol]).valueOf();
          else if (parseAsNum)
            d = r[timecol];
          else
      ...
      Step(
        ...
        function (err, user) {
          ...
          var props = {
            ...
            sources: com.tagify(payload.sources, ','),
          ...
          db.Datasets.create(props, {force: {_id: 1, client_id: 1},
            ...
            // Index for search.
            com.index(self.search, doc, ['title', 'sources', 'tags']);
