...
var printSamples = require('../util/print_samples').printSamples;
var db = require('./db');
...
// Constructor
var Client = exports.Client = function (socket, pubsub, samples) {
  this.socket = socket;
  this.pubsub = pubsub;
...
Client.prototype.insertSamples = function (data, cb) {
  ...
  function parseTimecol(row) {
    ...
    _.each(row, function(val, key) {
      // isFinite checks for a valid date object
      if (isFinite(new Date(intRegex.test(val) ? parseInt(val) : val)))
        headerScore[key] += 1;
      else
    ...
    result.parseAsNum = intRegex.test(row[result.column]);
    return result;
  }

  Step(

    // selct our parsing mechanism based on file extension
    function () {
    ...
    function (err, rows, nn) {
      ...
      if (!checkIncreasingDate()) {
        ...
        if (!checkIncreasingDate())
          return this('Time is not sequential (forward or backward)');
      }
    ...
    function (err) {
      ...
      if (!self.socket.handshake.user)
        return this('No user found');
      var author_id = self.socket.handshake.user._id;
      ...
      // Create dataset.
      if (channels.length === 0) return this('No channels found');
    ...
    function (err) {
      ...
      self.pubsub.notify({actor_id: dataset.author._id, target_id: dataset._id,
          action_id: dataset._id, data: {
        action: {
      ...
      self.pubsub.subscribe(dataset.author, dataset, {style: 'watch', type: 'dataset'});

      // Broadcast to all clients.
      self.pubsub.publish('datasets', 'dataset.new', com.client(dataset));
