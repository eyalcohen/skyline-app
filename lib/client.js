...
Client.prototype.fetchSamples = function (did, channelName, options, cb) {
  ...
  this.sampleDbExecutionQueue(_.bind(function (done) {
    function next(err, realAndMathSamples) {
      ...
      var samples = realAndMathSamples.samples;
      cb(err, {
        samples: samples,
        range: {
        ...
        mathSamples: realAndMathSamples.mathSamples
      });
      done();
    };
...
function parseAndCheckFile(data, cb) {
  ...
  Step(
    ...
    function (err, rows, nn) {
      ...
      if (!rows || rows.length === 0) this('This dataset does not appear to have any valid data');
      var tmp = {};
  ...
  Step (
    ...
    function (err) {
      ...
      _.each(columns, function (channelName) {
        ...
        var tmp = m ? m[1]: channelName;
        var cn = _.slugify(_.prune(tmp.toLowerCase(), 40, '')).replace(/-/g, '_')
            + '__' + did;
        var humanName = payload.channels[channelName].humanName;
        if (humanName === '') humanName = tmp;
        var v = {
    ...
    function (err) {
      ...
      Step(
        ...
        // Step 2.  Add to the channels collection
        function (err, doc) {
          if (err) return next(err);
          user = doc;
          var group = this.group();
          _.each(schema, _.bind(function(s) {
            //if (!this.channelName) ;
            _.extend(s, { parent_id: did });
        ...
        function (err, docs) {
          _.each(docs, _.bind(function(d) {
            com.index(self.channelSearch, d, ['humanName'], this.parallel());
          }, this));
        },
        // Step 4.  Add to the dataset collection
        function (err) {
          if (err) return next(err);
