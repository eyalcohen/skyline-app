...
//  * Fetch channel range.
//  */
// Client.prototype.fetchChannelRange = function (did, channelName, cb) {

//   util.debug(color.bgBlackBright.white.bold('Worker '
//       + cluster.worker.id + ': fetchChannelRange:'));
...
//     function next(err, range) {
//       cb(err, range);
//       done();
//     };
//     this.samples.fetchChannelRange(did, channelName, next);
//   }, this));
// }
  ...
  this.sampleDbExecutionQueue(_.bind(function (done) {
    function next(err, samples) {
      cb(err, {
        samples: samples,
        range: {
          beg: options.beginTime || _.first(samples).beg,
          end: options.endTime || _.last(samples).end
        }
      });
      done();
    };
  ...
  Step(
    ...
    function (err, rows, nn) {
      ...
      _.each(rows, _.bind(function (row, index) {
        var line = index + 1;
        var end = timecolGuess.parseAsNum ? row[timecol] : new Date(row[timecol]).valueOf();
        end = end * 1e3;
