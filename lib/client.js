...
Client.prototype.sendPartialFile = function (data, cb) {
  ...
  if (chunk[id].currentSize == chunk[id].expectedSize) {
    ...
    this.previewFileInsertion({ fileId: id }, cb);
  } else {
    cb(null, {
...
var dateFormats =  [
  { fmt: 'YY', example: '84'},
  { fmt: 'YYYY', example: '1984'},
  { fmt: 'MM-DD-YY', example: '5/23/84'},
  { fmt: 'MM-DD-YYYY', example: '5/23/1984'},
  { fmt: 'DD-MM-YY', example: '23/5/84'},
  { fmt: 'DD-MM-YYYY', example: '23/5/1984'},
  { fmt: 'MMM-YYYY', example: 'May 1984'},
  { fmt: 'MMM-DD-YYYY', example: 'May 23 1984'},
  { fmt: 'DD-MMM-YYYY', example: '23 May 1984'},
  { fmt: null, example: 'ISO-8601'},
  { fmt: null, example: 'Unix time'},
  { fmt: null, example: 'Skyline determined'}
]

var timeFormats = [
  { fmt: 'HH', example: '16' },
  { fmt: 'hh', example: '4' },
  { fmt: 'hh a', example: '4 PM' },
  { fmt: 'HH:mm', example: '16:30' },
  { fmt: 'hh:mm', example: '4:30' },
  { fmt: 'hh:mm a', example: '4:30 PM' },
  { fmt: 'HH:mm-ss', example: '16:30:45' },
  { fmt: 'hh:mm-ss', example: '4:30:45' },
  { fmt: 'hh:mm-ss a', example: '4:30:45 PM' },
  { fmt: 'HH:mm-ss.SSS', example: '16:30:45.10' },
  { fmt: 'hh:mm-ss.SSS', example: '4:30:45.10' },
  { fmt: 'hh:mm-ss.SSS a', example: '4:30:45.10 PM' },
  { fmt: 'mm', example: '30' },
  { fmt: 'mm ss', example: '30:45' },
  { fmt: 'ss', example: '45 seconds' },
  { fmt: 'ss.sss', example: '45.10 seconds' },
  { fmt: 'sss', example: '10 miliseconds' },
];
  ...
  Step(
    ...
    function (err, rows, nn) {
      ...
      if (!obj.dateColumn || !obj.dateFormat) {
        ...
        obj.dateColumn = timecolGuess.column;
        obj.dateFormat = timecolGuess.dateHint;
      }

      var ret = {};
      var problemRow = null;

      _.each(rows, function(r, idx) {
        var m;
        if (obj.dateFormat)
          m = moment(r[obj.dateColumn], obj.dateFormat);
        else
          m = moment(r[obj.dateColumn]);

        if (m.isValid()) {
          r[obj.dateColumn] = m.format();
        }
        else {
          r[obj.dateColumn] = null;
          if (!problemRow) {
            problemRow = idx;
          }
        }
      ...
      var ret = {
        ...
        firstRows: _.first(rows, 5),
        lastRows: _.last(rows, 5),
    ...
    function (err, rows, nn) {
      ...
      function checkIncreasingDate() {
        ...
        var badmatch = _.find(rows, function(row) {
          ...
          var dur = end - prevEnd;
          if (dur < 0) {
            failingDate = {date: row[timecol], row: timecol};
            return date;
          }
          prevEnd = end;
        })
      ...
      if (!checkIncreasingDate()) {
        ...
        var prevFailingDate = _.extend(failingDate);
        console.log(prevFailingDate);
        if (!checkIncreasingDate())
          return this('Time is not always increasing or decreasing.');// +
                      //prevFailingDate.row ? 'Check row:' + prevFailingDate[row] : '');
      }
