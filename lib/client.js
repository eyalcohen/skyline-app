...
var dateFormats = {
  'Let Skyline guess': { fmt: null,                         example : ''},
  'm-d-y':             { fmt: ['MM-DD-YY', 'MM-DD-YYYY'],   example : '5/23/84'},
  'd-m-y':             { fmt: ['DD-MM-YY', 'DD-MM-YYYY'],   example : '23/5/84'},
  'y':                 { fmt: ['YY', 'YYYY'],               example : '84, 1984'},
  'm':                 { fmt: ['MM'],                       example : '5'},
  'd':                 { fmt: ['DD'],                       example : '23'},
...
Client.prototype.previewFileInsertion = function (obj, cb) {
  ...
  Step(
    ...
    function (err, rows, nn) {
      ...
      var dateColumn, timeColumn, dateFormatKey, timeFormat;
      ...
      // If we have an additional time column, set a variable
      } else if (obj.timeColumn && obj.timeFormat) {
        dateColumn = obj.dateColumn;
        dateFormatKey = obj.dateFormat
      // If we have a time format, combine it with the date format
      // eg: '5/23/84 10:30 pm'
      } else if (obj.timeFormat) {
        dateColumn = obj.dateColumn;
        dateFormatKey = obj.dateFormat
      // Otherwise this is date only, no time
      } else {
        dateColumn = obj.dateColumn;
        dateFormatKey = obj.dateFormat
      }
      ...
      _.find(rows, function(r, idx) {
        ...
        if (dateFormats[dateFormatKey] && dateFormats[dateFormatKey].fmt) {
          m = moment(r[dateColumn], dateFormats[dateFormatKey].fmt);
        } else {
      ...
      var ret = {
        ...
        firstRows: _.first(rows, 4),
        lastRows: _.last(rows, 4),
