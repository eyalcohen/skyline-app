    ...
    Step(
      function () {
        self.db.collection(name, _.bind(function (err, col) {
          ...
          collection = col;
          exports[_.capitalize(collection.collectionName) + 's'] = collection;
          if (options.ensureIndexes && cluster.worker.id === 1) {
            util.log('Worker ' + cluster.worker.id + ': Ensuring `'
                + collection.collectionName + '` collection indexes');
            collection.dropAllIndexes(_.bind(function () {
              var next = _.after(conf.indexes.length, this);
              _.each(conf.indexes, function (x, i) {
                collection.ensureIndex(x, {unique: conf.uniques[i],
                    dropDups: true}, next);
              });
            }, this));
          } else this();
      ...
      function (err) { cb(err, collection); }
    );
  };
