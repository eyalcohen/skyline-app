...
Samples.prototype.insertSamples = function(datasetId, sampleSet, options, cb) {
  ...
  Step(
    ...
    function finishInserting(err) {
      ...
      self.insertGeospatial(datasetId, sampleSet, function (err) {
        if (err) {
          console.log('Error in insertGeospatial while inserting: ' +
                      (err.stack || err));
        }
      });
...
Samples.prototype.insertGeospatial = function(datasetId, sampleSet, cb) {
  var self = this;
  if (!cb) {
    cb = function (){};
  }
  var channelNames = Object.keys(sampleSet);
  function _find(key) {
    return _.find(channelNames, function (cn) {
      return _.str.include(cn, key);
    });
  }
  ...
  var lats = sampleSet[lat];
  var lons = sampleSet[lon];
  var samples = [];
  if (lats && lons) {
    for (var i = 0; i < Math.min(lats.length, lons.length); ++i) {
      samples.push({
        latitude: lats[i].val,
        longitude: lons[i].val,
        beg: lats[i].beg,
        end: lats[i].end
      });
    }
  ...
  var names = ["the_geom", "id", "tb", "te"];
  var queryInit = "INSERT INTO " + self.cartodb.table
      + " (" + _.join(",", names) + ") VALUES ";
  // make a batch set of CURL queries
  var per = 100;
  var query = [];
  for (i = 0; i < samples.length; i += per) {
    query[i/per] = queryInit;
    var sliced = samples.slice(i, i + per);
    _.each(sliced, function (s, j) {
      query[i/per] += "(" + _.join(",", [
          "CDB_LatLng(" + s.latitude + "," + s.longitude + ")",
          datasetId,
          s.beg,
          s.end,
          // "'" + new Date(s.beg/1e3).toISOString() + "'",
          // "'" + new Date(s.end/1e3).toISOString() + "'",
          ]) + ")";
      if (j !== Math.min(per, sliced.length) - 1) {
        query[i/per] += ", ";
      }
    });
  }

  // Send samples to CartoDB.
  Step(
    function () {
      var group = this.group();
      _.each(query, function(_query) {
        curl.request({
          url: 'https://' + self.cartodb.user + '.cartodb.com/api/v2/sql',
          method: 'POST',
          data: {q: _query, api_key: self.cartodb.key}
        }, group());
      });
    },
    function (err, data) {
      ...
      _.each(sampleSet, function(samples, channelName) {
        ...
        if (first && first.beg == first.end) {
          ...
          self.fetchSamples(datasetId, channelName, options,
            ...
            if (err)
              console.log('Error in addDurationHeuristicHack while fetching: ' +
                  (err.stack || err));
            next();
          });
        }
