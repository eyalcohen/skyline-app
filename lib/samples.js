...
Samples.prototype.fetchRealSamples =
  ...
  Step(
    ...
    function(err) {
      ...
      if (!options.math) {
        cb(err, { samples: samples, mathSamples: null });
      } else {
        cb(err, { samples: samples, mathSamples: new SampleMath(samples, self, options.math)});
      }
    }
  );
}
...
Samples.prototype.fetchMergedSamples =
  ...
  Step(
    ...
    function(err, realAndMathSamples, synSamples) {
      var realSamples = realAndMathSamples.samples;
...
Samples.prototype.fetchSamples =
  ...
  function fetchInternal(cb) {
    ...
    else if (options.type == 'real' || !options.minDuration || options.map)
