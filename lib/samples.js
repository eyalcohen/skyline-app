  ...
  Step(
    ...
    function () {
      ...
      _.each(bucketThresholds, function (level) {
        var next = genNext();
        app.get('connection').add('real_' + level, {
      ...
      _.each(syntheticDurations, function (duration) {
        var next = genNext();
        app.get('connection').add('syn_' + duration, {
          indexes: [{did: 1}, {chn: 1}, {buk: 1}],
          uniques: [false, false, false]
        }, function (err, collection) {
 ...
 */
var sampleValuesEqual = function(val1, val2) {
...
// var bucketThresholds = Samples.bucketThresholds;
// var syntheticDurations = Samples.syntheticDurations;
// var syntheticSamplesPerRow = Samples.syntheticSamplesPerRow;
 ...
 */
Samples.resample = function(inSamples, beg, end, sampleSize, options) {
  options = options || {};
  _.defaults(options, {
    stddev: false,
    keepAll: false,
